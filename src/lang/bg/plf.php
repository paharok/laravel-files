<?php
//bg
return [
    "questRemoveFile"=>"Сигурни ли сте, че искате да изтриете този файл?",
    "questRemoveDir"=>"Сигурни ли сте, че искате да изтриете тази директория?",
    "createFolder"=>"Създайте директория",
    "uploadFiles"=>"Изтегляне на файлове",
    "search"=>"Търсене",
    "create"=>"Създавайте",
    "cancel"=>"Отказ",
    "fileName"=>"Името на файла",


    "errorEmptyField"=>"Празно поле",
    "errorSomethingWrong"=>"Нещо се обърка!",
    "errorInvalidPath"=>"Невалиден път",
    "questRenameFile"=>"Преименуване на файл",
    "questRenameDir"=>"Преименуване на директория",

    "It's same name!"=>"Това е същото име!",
    "rename_exists"=>":newname вече съществува. Новото име е :newnameuniq",
    "No such file or directory"=> "Не съществува такъв файл или директория",
    "Delete"=>"Изтриване на",
    "Insert"=>"Вмъкване на",
    "Move"=>"Преместване",
    "Copy"=>"Копие",
    "Are you sure you want to delete the selected files?"=>"Сигурни ли сте, че искате да изтриете избраните файлове?",
];
